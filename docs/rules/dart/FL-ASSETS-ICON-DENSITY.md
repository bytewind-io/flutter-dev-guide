...
ai_hint: >
  ...
  3) НЕ флагируй: векторные изображения (svg), анимации, другие ресурсы (не иконки).
  ...
  6) Фокусируйся на иконках UI (кнопки, навигация, действия), а не на больших изображениях контента.
...
tests:
  ...
  should_pass:
    ...
    - path: "assets/images/logo.svg"
      content: |
        # Векторное изображение не требует версий для разных плотностей
        # ✅ SVG масштабируется автоматически
...
1. **Векторные изображения (SVG)**: Автоматически масштабируются без потери качества
   ```yaml
   flutter:
     assets:
       - assets/icons/logo.svg  # ✅ Не требует версий 2x/3x
   ```

2. **Большие изображения контента**: Фотографии, баннеры (достаточно одной версии высокого разрешения)
   ```yaml
   flutter:
     assets:
       - assets/images/banner.jpg  # ✅ Можно использовать одну версию
   ```

3. **Анимации и видео**: Форматы, которые сами управляют разрешением
...
4. **Векторизируйте где возможно**: Предпочитайте SVG для простых иконок
...
- [ ] Векторные иконки используют формат SVG вместо растровых версий
